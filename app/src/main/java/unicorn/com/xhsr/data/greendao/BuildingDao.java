...
public class BuildingDao extends AbstractDao<Building, String> {
    ...
    */
    ...
    private Query<Building> building_ChildrenQuery;
    ...
    /** Internal query to resolve the "children" to-many relationship of Building. */
    public List<Building> _queryBuilding_Children(String parentId) {
        synchronized (this) {
            if (building_ChildrenQuery == null) {
                QueryBuilder<Building> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.ParentId.eq(null));
                queryBuilder.orderRaw("T.'ORDER_NO' ASC");
                building_ChildrenQuery = queryBuilder.build();
            }
        }
        Query<Building> query = building_ChildrenQuery.forCurrentThread();
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(" LEFT JOIN BUILDING T0 ON T.\"PARENT_ID\"=T0.\"OBJECT_ID\"");
            builder.append(' ');
    ...
    protected Building loadCurrentDeep(Cursor cursor, boolean lock) {
        ...
        Building parent = loadCurrentOther(daoSession.getBuildingDao(), cursor, offset);
